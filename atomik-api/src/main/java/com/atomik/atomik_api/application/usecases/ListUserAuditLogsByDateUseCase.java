...
@Service
public class ListUserAuditLogsByDateUseCase {
    private final UserRepository userRepository;
    private final AuditLogRepository auditLogRepository;

    public ListUserAuditLogsByDateUseCase(UserRepository userRepository, AuditLogRepository auditLogRepository) {
        this.userRepository = userRepository;
        this.auditLogRepository = auditLogRepository;
    }

    public List<AuditLogResponseDTO> execute(String userId, LocalDate startDate, LocalDate endDate) {
        userRepository.findById(UUID.fromString(userId)).orElseThrow(() -> new UserNotFoundException("User not found"));

        var auditLogs = auditLogRepository.findByUserIdAndDateBetween(userId, startDate, endDate);
